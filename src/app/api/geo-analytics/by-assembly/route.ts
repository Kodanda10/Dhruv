...
export async function GET(request: NextRequest) {
  try {
    ...
    const fullWhereClause = `${baseWhereClause}${geoWhereClause} AND pe.geo_hierarchy IS NOT NULL`;
    ...
    const totalEvents = blocksRows.reduce((sum, row) => sum + parseInt(row.event_count || '0'), 0);
