...
async function get<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  const ok = (res as any).ok === undefined ? true : (res as any).ok;
  if (!ok) throw new Error(`GET ${path} failed: ${res.status}`);
...
async function put<T>(path: string, body: any): Promise<T> {
  ...
  const ok = (res as any).ok === undefined ? true : (res as any).ok;
  if (!ok) throw new Error(`PUT ${path} failed: ${res.status}`);
  return res.json() as Promise<T>;
}
...
async function post<T>(path: string, body?: any): Promise<T> {
  ...
  const ok = (res as any).ok === undefined ? true : (res as any).ok;
  if (!ok) throw new Error(`POST ${path} failed: ${res.status}`);
  return res.json() as Promise<T>;
}
